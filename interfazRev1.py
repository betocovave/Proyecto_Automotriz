...
class AutomotrizApp:
    def __init__(self, root):
        ...
        self.mechanics_listbox = CTkTextbox(self.admin_frame, width=400, height=100)
        self.mechanics_listbox.pack(pady=10)
    ...
    def view_clients(self):
        ...
        if rows:
            client_list = "\n".join([f"ID: {row[0]}, Nombre: {row[1]}, Correo: {row[2]}, Modelo: {row[3]}, Revisión: {row[4]}, Próxima Visita: {row[5]}" for row in rows])
    ...
    def update_mechanics_list(self):
        ...
        rows = self.c.fetchall()
        self.mechanics_listbox.delete("1.0", "end")
        for row in rows:
            self.mechanics_listbox.insert("end", f"ID: {row[0]}, Usuario: {row[1]}\n")
