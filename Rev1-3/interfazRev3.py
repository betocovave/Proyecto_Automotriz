...
from tkinter import filedialog, messagebox, simpledialog, ttk, END
import sqlite3
...
class AutomotrizApp:
    def __init__(self, root):
        ...
        self.user_label = CTkLabel(self.sidebar, text="Usuario: ", font=("Arial", 14))
        self.user_label.pack(pady=20)

        # Switch para cambiar entre modos claro y oscuro
        self.switch = CTkSwitch(self.sidebar, text="Dark Mode",
                                onvalue=1, offvalue=0,
                                command=self.change_mode)
        self.switch.pack(pady=20)

        # Botón para cerrar sesión
        self.logout_button = CTkButton(self.sidebar, text="Cerrar Sesión", command=self.logout)
        ...
        # Notebook (pestañas) para clientes y mecánicos
        self.notebook = ttk.Notebook(self.content_frame)
        self.notebook.pack(fill="both", expand=True)

        # Pestaña de clientes
        self.client_tab = CTkFrame(self.notebook)
        self.notebook.add(self.client_tab, text="Clientes")

        # Pestaña de mecánicos (solo para admin)
        self.mechanic_tab = CTkFrame(self.notebook)
        self.notebook.add(self.mechanic_tab, text="Mecánicos")

        # Frame para el análisis predictivo
        self.analysis_frame = CTkFrame(self.client_tab)
        ...
        self.client_frame = CTkFrame(self.client_tab)
        ...
        self.client_tree.grid(row=5, column=0, columnspan=2, pady=10)

        # Frame para la imagen térmica
        self.thermal_frame = CTkFrame(self.client_tab)
        ...
        self.mechanic_frame = CTkFrame(self.mechanic_tab)
        self.mechanic_frame.pack(pady=20)

        self.mechanics_label = CTkLabel(self.mechanic_frame, text="Mecánicos Registrados", font=("Arial", 16))
        self.mechanics_label.pack(pady=10)

        self.mechanics_tree = ttk.Treeview(self.mechanic_frame, columns=("ID", "Usuario"), show="headings")
        self.mechanics_tree.heading("ID", text="ID")
        self.mechanics_tree.heading("Usuario", text="Usuario")
        self.mechanics_tree.pack(pady=10)

        self.add_mechanic_button = CTkButton(self.mechanic_frame, text="Agregar Mecánico", command=self.add_mechanic)
        self.add_mechanic_button.pack(pady=10)

        self.remove_mechanic_button = CTkButton(self.mechanic_frame, text="Quitar Mecánico", command=self.remove_mechanic)
    ...
    def login(self):
        ...
        if not usuario and not contraseña:
            ...
            if usuario == "admin" and contraseña == "1303":
                ...
                self.user_label.configure(text=f"Usuario: {usuario}")
    ...
    def logout(self):
        self.main_frame.pack_forget()  # Oculta el main_frame
        self.login_frame.pack(pady=20)  # Muestra el login_frame
        self.username_entry.delete(0, END)  # Limpia el campo de usuario
        self.password_entry.delete(0, END)  # Limpia el campo de contraseña
