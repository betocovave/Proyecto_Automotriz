...
function exportAdvancedReport() {
    // Obtener formato de exportación y otras opciones
    ...
    let fileName = `reporte_talleres_${timestamp}`;
    
    // Obtener datos para el reporte
    getMaintenanceData(12).then(data => {
        // Preparar datos según nivel de detalle
        const reportData = prepareReportData(data, detailLevel);
        
        // Exportar según formato seleccionado
        switch (format) {
            case 'pdf':
                exportToPdf(reportData, fileName, includeGraphics);
                break;
            case 'excel':
...
// Preparar datos para el reporte según nivel de detalle
function prepareReportData(data, detailLevel) {
    // Filtrar y transformar datos según el nivel de detalle
    switch (detailLevel) {
        case 'summary':
            // Solo incluir resumen general
            const summaryData = {
                totalMantenimientos: data.length,
                costoTotal: data.reduce((sum, item) => sum + item.costo, 0),
                costoPromedio: data.reduce((sum, item) => sum + item.costo, 0) / data.length,
                preventivos: data.filter(item => item.tipo === 'preventivo').length,
                correctivos: data.filter(item => item.tipo === 'correctivo').length,
                fechaInforme: new Date().toLocaleDateString()
            };
            return { summary: summaryData, details: null };
            
        case 'detailed':
            // Incluir resumen y algunos detalles
            const detailedData = {
                summary: {
                    totalMantenimientos: data.length,
                    costoTotal: data.reduce((sum, item) => sum + item.costo, 0),
...
// Exportar a PDF
function exportToPdf(data, fileName, includeGraphics) {
    alert('La exportación a PDF está siendo preparada. Se descargará automáticamente cuando esté lista.');
    
    // En una implementación real, aquí se generaría el PDF
    // Podría usarse una biblioteca como jsPDF para la generación del PDF
    console.log('Exportando a PDF:', data);
    
    // Simular descarga después de un breve retardo
    setTimeout(() => {
        alert('Reporte PDF generado exitosamente. En una implementación real, se habría descargado.');
    }, 1500);
}

// Exportar a Excel
function exportToExcel(data, fileName) {
    alert('La exportación a Excel está siendo preparada. Se descargará automáticamente cuando esté lista.');
    
    // En una implementación real, aquí se generaría el archivo Excel
    // Podría usarse SheetJS (js-xlsx) para la generación del archivo Excel
    console.log('Exportando a Excel:', data);
    
    // Simular descarga después de un breve retardo
    setTimeout(() => {
        alert('Reporte Excel generado exitosamente. En una implementación real, se habría descargado.');
    }, 1500);
}

// Exportar a CSV
function exportToCsv(data, fileName) {
    // Crear contenido CSV a partir de los datos
    let csvContent = "data:text/csv;charset=utf-8,";
    
    // Encabezados del resumen
    csvContent += "Informe de Mantenimientos\r\n";
    csvContent += `Fecha del Informe,${data.summary.fechaInforme}\r\n`;
    csvContent += `Total Mantenimientos,${data.summary.totalMantenimientos}\r\n`;
    csvContent += `Costo Total,${data.summary.costoTotal}\r\n`;
    csvContent += `Costo Promedio,${data.summary.costoPromedio}\r\n`;
    csvContent += `Preventivos,${data.summary.preventivos}\r\n`;
    csvContent += `Correctivos,${data.summary.correctivos}\r\n\r\n`;
